...
TEST(surface_glx_driver_is_registered)
{
	...
	EXPECT_NOT_NULL(t_surface_glx_driver());
...
TEST(surface_glx_config_window_sets_config)
{
	START;
...
TEST(surface_glx_native_returns_display_and_visual)
{
	START;
...
TEST(surface_glx_gfx_proc_loads_symbol)
{
	...
	EXPECT_PTR(sym, t_surface_glx_symbol((t_surface_glx_symbol_t)t_glXSwapBuffers));
...
TEST(surface_glx_gfx_make_current_calls_glx)
{
	...
	EXPECT_PTR(t_glx_context, t_glx_create_context_ret);
...
TEST(surface_glx_gfx_clear_current_calls_glx)
{
	...
	EXPECT_EQ(t_glx_make_current_calls, 1);
	EXPECT_EQ(t_glx_drawable, 0);
	EXPECT_NULL(t_glx_context);
...
STEST(surface_glx)
{
	...
	RUN(surface_glx_config_window_sets_config);
	...
	RUN(surface_glx_native_returns_display_and_visual);
