...
{
	if (!surface_config_valid(config) || info == NULL || gfx_api(config->gfx, &info->gfx_api)) {
		return 1;
	}

	display_native_t native = {0};
	if (display_native(config->display, &native)) {
...
int surface_present(surface_t *srf)
{
	if (srf == NULL || srf->drv == NULL || srf->drv->present == NULL) {
		return 1;
	}

	return srf->drv->present(srf);
}
